<main id="contenido">  
  ...
  <?php foreach($consulta as $row) { ?>
    ...
    <?php if (is_null($row->respuesta)) { ?>
                <a title="Responder Consulta" class="img-carrito2 btn btn-dark" href="<?php echo base_url("administrador_controler/responderEstaConsulta/$row->id_consulta");?>" role="button"> Responder 📨</a>
                <?php }else{ ?>
                <div class="img-carrito2 bg-warning ml-auto mr-auto mt-4" style="text-align: center;">
                <h5 class="mt-4">Respuesta: <b class="text-capitalize"><?php echo $row->respuesta;?> </b></h5><br>
                <a title="Editar Respuesta" class="img-carrito2 btn btn-dark" href="<?php echo base_url("administrador_controler/responderEstaConsulta/$row->id_consulta");?>" role="button"> ✎ Editar Respuesta</a>
                </div>
    <?php } ?>
    <a title="Marcar como no leido" class="img-carrito2 btn btn-dark" href="<?php echo base_url("administrador_controler/leido_consulta_clientes/$row->id_consulta");?>" role="button">Marcar como no leido</a>
    <?php if ($row->archivo==1){?>
    <a title="Archivar Consulta" class="img-carrito2 btn btn-dark" href="<?php echo base_url("administrador_controler/archivar_consulta_clientes/$row->id_consulta");?>" role="button">💾 Archivar</a>
    <?php }else{?>
    <a title="Quitar del Archivo" class="img-carrito2 btn btn-dark" href="<?php echo base_url("administrador_controler/desarchivar_consulta_clientes/$row->id_consulta");?>" role="button">✘ Quitar del Archivo</a>
    <?php } ?>
    </div>
  <?php } ?>
  </div><br><br>
